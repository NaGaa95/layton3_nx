...
#define SLOT_BYTES (8 * 1024)   // private copy of one CRI chunk (~512 B each)
// CRI hands us tiny ~128-frame (2.7 ms) buffers; audout plays a handful of
// larger buffers more reliably, so several chunks are gathered into one ~21 ms
// device buffer before it is submitted.
#define SUBMIT_FRAMES 1024

typedef void (*slBufferQueueCallback)(void *bq, void *context);
typedef void (*slPlayCallback)(void *play, void *context, SLuint32 event);

// CRI's PCM is copied into the slot the instant it is enqueued, rather than
// holding a pointer into CRI's own buffer: CRI recycles those buffers on its
// server thread and would overwrite them before we read them, which glitched
// the audio at every chunk seam.
typedef struct {
...
typedef struct Player {
  ...
  // our private copies of the PCM the game enqueued
...
static u64 convert_to_device(Player *p, const int16_t *src, SLuint32 src_bytes, int16_t *dst, u64 dst_cap_bytes) {
  ...
  const int in_rate = p->rate > 0 ? p->rate : out_rate;

  // fast path: ADX2 already mixes to 48 kHz stereo on this device, so just copy
  if (in_ch == 2 && in_rate == out_rate) {
    u64 bytes = (u64)in_frames * 4;
    if (bytes > dst_cap_bytes)
      bytes = dst_cap_bytes & ~3ull;
    memcpy(dst, src, bytes);
    return bytes;
  }

  // otherwise nearest-neighbour resample with a fixed-point step (16.16) so the
  // inner loop has no per-sample 64-bit divide
  int64_t out_frames = ((int64_t)in_frames * out_rate) / in_rate;
  ...
  const uint64_t step = ((uint64_t)in_frames << 16) / (uint64_t)out_frames;
  uint64_t pos = 0;
  for (int64_t i = 0; i < out_frames; i++) {
    const int64_t si = (int64_t)(pos >> 16);
    int16_t l, r;
    if (in_ch == 2) {
      l = src[si * 2 + 0];
      r = src[si * 2 + 1];
...
// block until a device buffer is free (the driver finished playing one) and
// return its index, or -1 if the player is shutting down. blocking here is what
// paces the thread to real time.
static int acquire_free_devbuf(Player *p) {
  while (p->running) {
    for (int i = 0; i < NUM_DEVBUFS; i++)
      if (!p->submitted[i])
        return i;
...
static void player_thread(void *arg) {
  ...
  int fill_idx = -1;  // device buffer we are accumulating chunks into
  u64 fill_bytes = 0;

  while (p->running) {
    reclaim_buffers(p);

    // wait for a queued chunk or shutdown
    mutexLock(&p->q_lock);
    ...
    const int slot = p->q_head;
    p->q_head = (p->q_head + 1) % QUEUE_LEN;
    mutexUnlock(&p->q_lock);
    // the slot stays valid until the producer wraps all the way around (64
    // enqueues away), long after we have converted it below
    const QueuedBuf *qb = &p->queue[slot];

    // make sure we have a device buffer to accumulate into; acquiring one
    // blocks on the driver, which is what paces this thread to real time
    if (fill_idx < 0) {
      fill_idx = acquire_free_devbuf(p);
      if (fill_idx < 0)
        break;
      fill_bytes = 0;
    }

    AudioOutBuffer *db = &p->devbuf[fill_idx];
    fill_bytes += convert_to_device(p, (const int16_t *)qb->buf, qb->size,
                                    (int16_t *)((uint8_t *)db->buffer + fill_bytes),
                                    db->buffer_size - fill_bytes);

    // the chunk is now in our device buffer; let the game refill its own
    if (p->bq_cb)
      p->bq_cb((void *)p->bqItf, p->bq_ctx);

    // submit once we have gathered enough to play seamlessly (or there is no
    // room left for another worst-case chunk)
    if (fill_bytes >= (u64)SUBMIT_FRAMES * 4 ||
        db->buffer_size - fill_bytes < (u64)qb->size * 2 + 16) {
      // overlay cutscene audio onto CRI's stream (single audout device)
      movie_mix_audio((int16_t *)db->buffer, (int)(fill_bytes / 4));
      db->data_offset = 0;
      db->data_size = fill_bytes;
      audoutAppendAudioOutBuffer(db);
      p->submitted[fill_idx] = 1;
      fill_idx = -1; // acquire a fresh buffer next time round
    }
  }
}
...
static SLresult bq_Enqueue(void *self, const void *pBuffer, SLuint32 size) {
  ...
  // copy now, while CRI still owns this buffer (it reuses it asynchronously)
  SLuint32 n = size > SLOT_BYTES ? SLOT_BYTES : size;
  memcpy(p->queue[p->q_tail].buf, pBuffer, n);
  p->queue[p->q_tail].size = n;
  p->q_tail = next;
