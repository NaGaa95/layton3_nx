/* imports.c -- .so import resolution table for libll3.so (Level-5 LT3R)
 *
 * Every undefined symbol libll3.so imports (per readelf --dyn-syms) is mapped
...
// pthread wrappers
//
// bionic stores these objects inline; the static initializers leave a sentinel
// in the first word (0=normal, 0x4000=recursive, 0x8000=errorcheck), so we make
// a real newlib object of that type and stash its pointer there. The type must
// come from a real mutexattr -- a *_INITIALIZER macro plus pthread_mutex_init(m,
// NULL) silently yields a NORMAL mutex and self-deadlocks the recursive locks.
//
// The vsync turnstile mutexes (see OS_Run) are locked on one thread and unlocked
// on another; libnx enforces ownership, so those two are routed through binary
// semaphores instead.
// ---------------------------------------------------------------------------
...
// newlib exposes toupper as a macro, so it has no address to hand the engine
static int toupper_fake(int c) { return toupper(c); }

// newlib's clock_nanosleep prototype is gated off here; emulate it over
// clock_gettime + nanosleep (TIMER_ABSTIME = sleep until the absolute time)
...
DynLibFunction dynlib_functions[] = {
  ...
  { "toupper", (uintptr_t)&toupper_fake },
