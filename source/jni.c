/* jni.c -- fake JNI environment for libll3.so (Level-5 LT3R)
 *
 * libll3.so talks to the "Java" MainActivity entirely through the JNIEnv we
...
// the engine picks its assets/data-XX language folder from this code; the
// values match the Vita port (1=EN, 2=ES, 3=FR, 4=IT, 5=DE)
static int device_language(void) {
  u64 code = 0;
  ...
  switch (lang) {
    case SetLanguage_ES:
    case SetLanguage_ES419: return 2; // Spanish
    case SetLanguage_FR:
    case SetLanguage_FRCA:  return 3; // French
    case SetLanguage_IT:    return 4; // Italian
    case SetLanguage_DE:    return 5; // German
    default:                return 1; // English (and anything unsupported)
  }
}
...
static juint call_int(FakeID *id, va_list va) {
  ...
  if (!strcmp(name, "LVL_GetState") || !strcmp(name, "LSH_GetState") ||
    ...
    return 2;
  if (!strcmp(name, "OS_GetDeviceLanguage"))
...
static void *call_object(FakeID *id, va_list va) {
  ...
  if (!strcmp(name, "OS_GetAppVersion"))
    return singleton_str(&s_version, "1.0.3");
